...
@dp.inline_handler()
async def search_products(query: types.InlineQuery):
    text = query.query
    products = await db.search_products_by_title(word=text)
    results = []
    for product in products:
        results.append(
            types.InlineQueryResultArticle(
                ...
                title=product['name'],
                input_message_content=types.InputTextMessageContent(
                    message_text="Botga o'tish: https://t.me/onlineStore0101_bot",
                ),
                url="https://t.me/onlineStore0101_bot",
